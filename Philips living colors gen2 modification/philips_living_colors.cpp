...
//Constructor
LedstripColor::LedstripColor(MySensor mySensorGw, int pin, int childId, int intDelay){
  ...
  gw = mySensorGw;
  ledPin = pin;
  //pinMode(ledPin, OUTPUT); //Not needed?
  mySensorChildId = childId;
  intervalDelay = intDelay;
  
  dimmerMsg = MyMessage(mySensorChildId, V_DIMMER);
  lightMsg = MyMessage(mySensorChildId, V_LIGHT);
}
...
void LedstripColor::Update(){
  ...
  if((currentLedLevel != desiredLedLevel) && (currentMillis - previousMillis >= intervalDelay)){
    ...
    gw.send(lightMsg.set(currentLedLevel > 0 ? 1 : 0));
    // hek comment: Is this really nessesary?
    gw.send(dimmerMsg.set(currentLedLevel));
  }
}
